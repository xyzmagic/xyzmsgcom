...
Alternatively, you can reference an up鈥恡o鈥恉ate version on our CDN:
...
constructor and you鈥檙e ready to go.
...
<script src="https://player.vimeo.com/api/player.js"></script>
<script>
    // If you want to control the embeds, you鈥檒l need to create a Player object.
...
If you鈥檙e using a module bundler like [webpack](https://webpack.js.org) or
...
Similarly, if you鈥檙e using [RequireJS](http://www.requirejs.org) in the browser,
...
at a later time. This is useful for situations where the player embed wouldn鈥檛
...
that event if a `callback` isn鈥檛 passed, or only that specific callback if it is
passed.
...
// If later on you decide that you don鈥檛 need to listen for play anymore.
...
When set via the API, the track language will not change the viewer鈥檚 stored
preference.
...
Pause the video if it鈥檚 playing.
...
Play the video if it鈥檚 paused. **Note:** on iOS and some other mobile devices,
...
player.removeCuePoint('09ecf4e4-b587-42cf-ad9f-e666b679c9ab').then(function(id) {
    ...
    switch (error.name) {
        ...
        case 'InvalidCuePoint':
            // a cue point with the id passed wasn鈥檛 found
            break;
...
Get the native width of the currently鈥恜laying video. The width of the highest
...
Get the native height of the currently鈥恜laying video. The height of the highest
resolution available will be used before playback begins.
...
player.getVideoUrl().then(function(url) {
    ...
    switch (error.name) {
        case 'PrivacyError':
            // the url isn鈥檛 available because of the video鈥檚 privacy setting
            break;
    ...
    cues: [
        {
            ...
            text: "<i>Here at Vimeo, there's always 鈫祇ne thing on our minds:</i>"
        }
    ],
    kind: "captions",
    label: "English CC",
...
autoplay    | `false`  | Automatically start playback of the video. Note that this won鈥檛 work on some devices.
