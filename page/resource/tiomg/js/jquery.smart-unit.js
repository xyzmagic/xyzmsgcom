...
$.widget( "custom.smartunit", {
  ...
  _create: function() {
    ...
    if(units){
     ...
     var tr = $("<tr/>");
      ...
      var smartunit_table_id = this.element.attr("id") + "_table";
      this.option({"smartunit_table_id": smartunit_table_id});
      var table = $("#" + smartunit_table_id);
      if(table){
        table.remove();
      }
      
      var table = $("<table />");
      this.element.after(table);
      
      table.attr("id", smartunit_table_id);
      table.append(tr);
 ...
 /**     
  ...
  val: function(value){
      var tableId = this.option("smartunit_table_id");
      ...
      if(arguments.length == 0){
          ...
          for(var i=0; i<inputs.length; i++){
              quantity_json_data[i] = {
                ...
                quantity_show: $(inputs[i]).val() + units[i].unit_name
              };
          }
         
         var vJSON = JSON.stringify(quantity_json_data);
         this.element.val(vJSON);
